...
static void* os_mmap(size_t size, size_t alignment, size_t* offset, size_t* mapped_size) 
{
	...
	void* ptr = VirtualAlloc(0, map_size, (os_huge_pages ? MEM_LARGE_PAGES : 0) | MEM_RESERVE | do_commit, PAGE_READWRITE);
	...
	*mapped_size = map_size;
	...
	size_t page_mapped_current = atomic_fetch_add_explicit(&global_statistics.page_mapped, page_count, memory_order_relaxed) + page_count;
...
static int os_mdecommit(void* address, size_t size)
{
	...
	size_t page_active_current = atomic_fetch_sub_explicit(&global_statistics.page_active, page_count, memory_order_relaxed);
...
static page_t* heap_get_page_generic(heap_t* heap, uint32_t size_class)
{
	...
	if (EXPECTED(page != 0))
	{
		...
		return heap_make_free_page_available(heap, size_class, page) != 0 ? 0 : page;
	}
...
static NOINLINE RPMALLOC_ALLOCATOR void* heap_allocate_block_small_to_large(heap_t* heap, uint32_t size_class, unsigned int zero)
{
...
static NOINLINE RPMALLOC_ALLOCATOR void* heap_allocate_block_huge(heap_t* heap, size_t size, unsigned int zero)
{
	...
	/*
	*  Edit note: The block null check is inverted in the original 
	* rpmalloc.c source.
	*/
...
static RPMALLOC_ALLOCATOR void* heap_allocate_block_aligned(heap_t* heap, size_t alignment, size_t size, unsigned int zero)
{
...
static void* heap_reallocate_block(
	heap_t* heap, 
	void* block, 
	size_t size, 
	size_t old_size, 
...
static void* heap_reallocate_block_aligned(
	heap_t* heap, 
	void* block, 
	size_t alignment, 
	size_t size, 
...
rpmalloc_heap_t* rpmalloc_get_heap_for_ptr(void* ptr)
{
	...
	return span ? span_get_page_from_block(span, ptr)->heap : 0;
}
